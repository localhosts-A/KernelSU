...
extern const unsigned char embedded_ksud_start[] __attribute__((weak));
extern const unsigned char embedded_ksud_end[] __attribute__((weak));
...
static size_t embedded_ksud_size(void) {
  uintptr_t start = (uintptr_t)embedded_ksud_start;
  uintptr_t end = (uintptr_t)embedded_ksud_end;
...
static int write_embedded_ksud_file(const char *path) {
  size_t size = embedded_ksud_size();
  if (!size) {
    errno = ENOENT;
    return 0;
  }

  int fd = open(path, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0755);
  if (fd < 0) {
    return 0;
  }
  int ok = write_all_fd(fd, embedded_ksud_start, size);
...
static const char *prepare_ksud_path(void) {
  const char *env_path = getenv("KSUD_PATH");
  if (env_path && *env_path) {
    if (access(env_path, X_OK) == 0) {
      return env_path;
    }
    write_embedded_ksud_file(env_path);
    return env_path;
  }

  const char *extract_path =
      env_or_default("KSUD_EXTRACT_PATH", "/data/local/tmp/ksud");
  if (write_embedded_ksud_file(extract_path) || access(extract_path, X_OK) == 0) {
    return extract_path;
  }
...
static int start_ksud_late_load(pid_t *launcher_pid) {
  if (launcher_pid) {
    *launcher_pid = -1;
  }
  if (!env_bool_default("KSUD_LATE_LOAD", 1)) {
    pr_info("ksud late-load skipped by KSUD_LATE_LOAD=0\n");
    errno = 0;
    return 1;
  }

  const char *ksud = prepare_ksud_path();
  const char *pkg = env_or_default("KSUD_PACKAGE", "me.weishu.kernelsu");
  const char *kmi = getenv("KSUD_KMI");
  int allow_shell = env_bool_default("KSUD_ALLOW_SHELL", 1);

  int exec_error_pipe[2] = {-1, -1};
  if (pipe(exec_error_pipe) != 0) {
    return 0;
  }
  fcntl(exec_error_pipe[0], F_SETFD, FD_CLOEXEC);
  ...
  if (pid == 0) {
    ...
    argv[argc++] = "late-load";
    if (allow_shell) {
      argv[argc++] = "--allow-shell";
    }
    argv[argc++] = "--package-name";
    argv[argc++] = (char *)pkg;
    if (kmi && *kmi) {
      argv[argc++] = "--kmi";
      argv[argc++] = (char *)kmi;
    ...
    argv[argc] = NULL;
    execv(ksud, argv);
    int exec_errno = errno ? errno : 127;
  ...
  ssize_t got;
  do {
    got = read(exec_error_pipe[0], &exec_errno, sizeof(exec_errno));
  } while (got < 0 && errno == EINTR);
  close(exec_error_pipe[0]);

  if (got == 0) {
    pr_success("ksud late-load launched pid=%d path=%s package=%s allow_shell=%d\n",
               pid, ksud, pkg, allow_shell);
    errno = 0;
    return 1;
  }
  ...
  if (got == (ssize_t)sizeof(exec_errno)) {
    ...
    errno = exec_errno;
    pr_warning("ksud late-load exec failed pid=%d path=%s errno=%d\n",
               pid, ksud, errno);
    return 0;
  }
  ...
  errno = EIO;
  pr_warning("ksud late-load launch failed pid=%d path=%s errno=%d\n",
             pid, ksud, errno);
  return 0;
}

static void cleanup_root_child_state(int kill_child_process) {
  if (kill_child_process && root_child_pid > 0) {
...
int spawn_root_child(void) {
  ...
  if (root_child_pid == 0) {
    ...
    for (int i = 0; i < ROOT_CHILD_WAIT_LOOPS; i++) {
      if (atomic_load(&root_shared->go)) {
        break;
      }
      usleep(ROOT_CHILD_POLL_USEC);
    }
    if (!atomic_load(&root_shared->go)) {
      _exit(2);
    }
    ...
    report.ksud_launcher_pid = -1;
    if (report.setgid_ret == 0 && report.setuid_ret == 0) {
      errno = 0;
      report.ksud_late_load_ret =
          start_ksud_late_load(&report.ksud_launcher_pid);
      report.ksud_late_load_errno = errno;
    } else {
      report.ksud_late_load_ret = 0;
      report.ksud_late_load_errno = EPERM;
    }
...
int install_android_root(int fd) {
  ...
  SYSCHK(clock_gettime(CLOCK_MONOTONIC, &start));
  int ok = 0;
...
#else
  pr_info("root selinux direct write ok=%d before=%u\n",
          selinux_direct_ok, selinux_before);
#endif
  ...
  pr_info("root child result done=%d uid_after=%u setgid=%d/%d setuid=%d/%d "
          "setenforce=%d/%d ksud_late_load=%d/%d launcher=%d selinux=%u->%u "
          "cap=%016llx/%016llx\n",
          root_child_done, root_uid_after, report.setgid_ret,
          report.setgid_errno, report.setuid_ret, report.setuid_errno,
          setenforce_ret, setenforce_errno, report.ksud_late_load_ret,
          report.ksud_late_load_errno, report.ksud_launcher_pid,
  ...
  pr_info("root child result done=%d uid_after=%u setgid=%d/%d setuid=%d/%d "
          ...
          selinux_before, selinux_after);
#endif
  ok = root_child_done && (report.ksud_late_load_ret || selinux_after == 0);
  cleanup_root_child_state(!ok);

  struct timespec end_ok;
  SYSCHK(clock_gettime(CLOCK_MONOTONIC, &end_ok));
  long elapsed_ms_ok = (end_ok.tv_sec - start.tv_sec) * 1000L +
                       (end_ok.tv_nsec - start.tv_nsec) / 1000000L;
  pr_info("root install elapsed_ms=%ld ok=%d fast_verify=%d\n", elapsed_ms_ok, ok,
#ifdef FAST_VERIFY
          1
#else
          0
...
fail: {
  cleanup_root_child_state(1);
  struct timespec end_fail;
  SYSCHK(clock_gettime(CLOCK_MONOTONIC, &end_fail));
  long elapsed_ms_fail = (end_fail.tv_sec - start.tv_sec) * 1000L +
                         (end_fail.tv_nsec - start.tv_nsec) / 1000000L;
  pr_info("root install elapsed_ms=%ld ok=0 fast_verify=%d\n", elapsed_ms_fail,
#ifdef FAST_VERIFY
          1
#else
          0
